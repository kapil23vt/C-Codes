...
int main() {
	...
	while (queries--) {
		...
		while (!q.empty()) {
			...
			// Removing the starting element from the queue
			// Since we are starting BFS from this point
			q.pop();

			//Going through all the adjacent nodes of that source node
			for (int i = 0; i < adjacent[u].size(); i++) {
				int v = adjacent[u][i];

				// IF node is not visited, distance updated
				// If dis is -1, we update it to -1+1
				// Since value is not -1 anymore, we mark that node as visited
				if (dis[v] == -1) {
					dis[v] = dis[u] + 1;
					q.push(v);
				}
